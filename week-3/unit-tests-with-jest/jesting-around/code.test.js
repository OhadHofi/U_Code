const CodeToCheck = require('./code')
const codeToCheck = new CodeToCheck()

test("add should return sum of a + b", () => {
    let sum = codeToCheck.add(1, 2)
    expect(sum).toBe(3)
})

test("calculateHyp should return the square root of the sum of the numbers’ squares", ()=>{
    let hypotenuse = codeToCheck.calculateHyp(3, 4)
    expect(hypotenuse).toBe(5)
})

test("should remove all BUGs from list of code", () => {
    let code = ["great code", "good code", "BUG", "async await awesome code", "BUG", "BUG", "general code"]
    let bugFreeCode = codeToCheck.removeBugs(code)
    expect(bugFreeCode).not.toContain("BUG")
    expect(bugFreeCode).toContain("good code")
})


test("should keep only HIGH priority tasks", () => {
    let tasks = [{ text: "dummy", priority: "HIGH" }, { text: "dummy", priority: "LOW" }]
    let taskWithOnlyHigh = codeToCheck.clearLowPriority(tasks)

    taskWithOnlyHigh.forEach(t => {
        expect(t.priority).toBe("HIGH")
    })
})
